...
def collision_prelim(rock_list,max_radius):
    ...
    nearest_pairs = list(tree.query_pairs(max_radius*10))
    #print nearest_pairs
...
def c_finder(rock_list):
    ...
    if (len(stuff) != 0):
        ...
        indices_of_colli_rocks = np.arange((len(new_rock_list) - amt_of_colli_rocks), len(new_rock_list), 1)
        #print "here comes the masses of the list"
            #for b in range(0, len(new_rock_list)):
                #print new_rock_list[b].mass
    return new_rock_list, collision_count, indices_of_colli_rocks
