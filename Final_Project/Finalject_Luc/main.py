...
def main(N=1000,moon_mass=3*7.35e22,moon_dist=384400000.,M=5.972e24, tstep=0, tmax=1.0e10, plot_int = 100.,filename='plot'):

	planet, rock_list, P_min = initial.initial(N,moon_mass,moon_dist,M)
	
	rock_list = oc.out_check(planet, rock_list)
	
	rock_list, collision_count_init, max_radius, max_mass = check.c_finder(rock_list)
	...
	for j in np.arange(0 , tstep + tmax, tstep):
		...
		rock_list = adv.advOdeInt(rock_list, tstep, planet)
		rock_list = oc.out_check(planet, rock_list)
		rock_list, collision_count, max_radius, max_mass = check.c_finder(rock_list)
		tot_col_cnt += collision_count
		#print tot_col_cnt
        
        #if ctr % 10 == 0:
        #	plot_c.gen_plot(rock_list,planet,moon_dist,orb_step=j,scale=3.)
		if ctr % 10 == 0:
			print 'total number of collisions ', tot_col_cnt
                        print 'number of rocks left: ', len(rock_list)
                        print 'largest radius/mass: ', max_radius, max_mass
			print 'time: ', j
		ctr += 1
